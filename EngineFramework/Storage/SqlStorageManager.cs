...
{
    public class SqlStorageManager : StorageManager
    {
        public T GetSetting<T>(string section, string property)
        {
            try
            {
       ...
       Value FROM dbo.Settings WHERE Section = @Section AND Property = @Property", connection);
        ...
        public void SaveSetting<T>(string section, string property, T value)
        {
